...
function ContextMenu() {
  ...
  if (menu.kind === 'file') {
    ...
    items.push({
      ...
      danger: true,
      onClick: () => {
        if (window.confirm('이 파일과 그 안의 모든 하위 파일·페이지를 삭제할까요?')) {
          void deleteFile(menu.id);
        }
      },
    });
  } else {
    ...
    items.push({
      ...
      danger: true,
      onClick: () => {
        if (window.confirm('이 페이지를 삭제할까요?')) {
...
function FileNode({ id, depth }: { id: string; depth: number }) {
  ...
  const isRenaming = renamingId === id;
  const hasChildren = file.childFileIds.length > 0 || file.pageIds.length > 0;
  ...
  return (
    <div>
      ...
      >
        <span className="file-tree-row-caret" aria-hidden>
      ...
      {isExpanded && (
        <div>
          {file.childFileIds.map((childId) => (
            <FileNode key={childId} id={childId} depth={depth + 1} />
          ))}
          {file.pageIds.map((pageId) => (
